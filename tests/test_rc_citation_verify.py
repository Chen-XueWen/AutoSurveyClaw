...
class TestVerifyByTitleSearch:
    ...
    def test_no_results_hallucinated(self) -> None:
        with patch("surveyclaw.literature.search.search_papers", return_value=[]):
...
class TestVerifyCitations:
    def test_full_pipeline_mocked(self) -> None:
        ...
        with (
            patch("surveyclaw.literature.verify.time.sleep"),
            patch("urllib.request.urlopen", side_effect=mock_urlopen),
            patch("surveyclaw.literature.search.search_papers", return_value=[]),
        ):
...
class TestStage23Integration:
    ...
    def test_stage_in_sequence(self) -> None:
        from surveyclaw.pipeline.stages import Stage, STAGE_SEQUENCE, NEXT_STAGE
    ...
    def test_contract_exists(self) -> None:
        from surveyclaw.pipeline.contracts import CONTRACTS
        from surveyclaw.pipeline.stages import Stage
    ...
    def test_executor_registered(self) -> None:
        from surveyclaw.pipeline.executor import _STAGE_EXECUTORS
        from surveyclaw.pipeline.stages import Stage

        assert Stage.CITATION_VERIFY in _STAGE_EXECUTORS

    def test_phase_map(self) -> None:
        from surveyclaw.pipeline.stages import PHASE_MAP, Stage
    ...
    def test_total_stages_is_23(self) -> None:
        from surveyclaw.pipeline.stages import STAGE_SEQUENCE
