...
from surveyclaw.overleaf.sync import OverleafSync
from surveyclaw.overleaf.conflict import ConflictResolver, _extract_conflicts, _resolve_content
from surveyclaw.overleaf.watcher import FileWatcher
from surveyclaw.overleaf.formatter import LatexFormatter
...
class TestLatexFormatter:
    ...
    def test_strip_local_comments(self) -> None:
        content = "Normal line\n% SURVEYCLAW: internal note\nAnother line\n"
        result = LatexFormatter.strip_local_comments(content)
        assert "SURVEYCLAW" not in result
...
class TestOverleafSync:
    ...
    @patch("surveyclaw.overleaf.sync.subprocess.run")
