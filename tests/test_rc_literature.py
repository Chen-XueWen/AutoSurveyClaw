...
"""Unit tests for surveyclaw.literature module.
...
from surveyclaw.literature.semantic_scholar import (
    _parse_s2_paper,
    search_semantic_scholar,
)
from surveyclaw.literature.arxiv_client import (
    _convert_result,
    search_arxiv,
)
from surveyclaw.literature.search import (
    _deduplicate,
    _normalise_title,
    papers_to_bibtex,
    search_papers,
...
class TestSemanticScholar:
    ...
    def test_search_semantic_scholar_mock(
        ...
        from surveyclaw.literature.semantic_scholar import _reset_circuit_breaker
        ...
        monkeypatch.setattr(
            "surveyclaw.literature.semantic_scholar.urllib.request.urlopen",
    ...
    def test_search_semantic_scholar_network_error(
        ...
        monkeypatch.setattr(
            "surveyclaw.literature.semantic_scholar.time.sleep", lambda _: None
        )
...
class TestArxiv:
    ...
    def test_search_arxiv_mock(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "surveyclaw.literature.arxiv_client.arxiv", _fake_arxiv,
        )
        monkeypatch.setattr(
            "surveyclaw.literature.arxiv_client._get_client",
            lambda: mock_client,
        )
        from surveyclaw.literature.arxiv_client import _reset_circuit_breaker
        _reset_circuit_breaker()
...
class TestSearchPapers:
    def test_search_papers_combines_sources(
        ...
        monkeypatch.setattr(
            "surveyclaw.literature.search.search_semantic_scholar",
            lambda *a, **kw: [s2_paper],
        )
        monkeypatch.setattr(
            "surveyclaw.literature.search.search_arxiv",
            lambda *a, **kw: [arxiv_paper],
        )
        monkeypatch.setattr("surveyclaw.literature.search.time.sleep", lambda _: None)
    ...
    def test_default_sources_openalex_first(self) -> None:
        ...
        from surveyclaw.literature.search import _DEFAULT_SOURCES
    ...
    def test_s2_failure_does_not_block_others(
        ...
        monkeypatch.setattr(
            "surveyclaw.literature.search.search_openalex",
            lambda *a, **kw: [],
        )
        monkeypatch.setattr(
            "surveyclaw.literature.search.search_semantic_scholar",
    ...
    def test_search_papers_multi_query(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "surveyclaw.literature.search.search_papers",
            mock_search,
        )
        monkeypatch.setattr("surveyclaw.literature.search.time.sleep", lambda _: None)
...
class TestArxivCircuitBreaker:
    ...
    def test_failure_triggers_circuit_breaker(self) -> None:
        ...
        from surveyclaw.literature import arxiv_client
...
class TestOpenAlex:
    def test_parse_openalex_response(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        from surveyclaw.literature.openalex_client import search_openalex
        ...
        monkeypatch.setattr(
            "surveyclaw.literature.openalex_client.urllib.request.urlopen",
            lambda *a, **kw: mock_resp,
        )
    ...
    def test_abstract_reconstruction(self) -> None:
        from surveyclaw.literature.openalex_client import _reconstruct_abstract
    ...
    def test_openalex_network_error(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        monkeypatch.setattr(
            "surveyclaw.literature.openalex_client.time.sleep", lambda _: None,
        )
...
class TestCacheTTL:
    def test_source_specific_ttl(self, tmp_path: Any) -> None:
        ...
        from surveyclaw.literature.cache import get_cached, put_cache, _SOURCE_TTL
    ...
    def test_citation_verify_ttl_is_permanent(self) -> None:
        from surveyclaw.literature.cache import _SOURCE_TTL
