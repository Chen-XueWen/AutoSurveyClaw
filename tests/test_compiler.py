"""Tests for surveyclaw.templates.compiler — BUG-197 and general compilation.
...
from surveyclaw.templates.compiler import (
    CompileResult,
    _is_fatal_error,
...
class TestSanitizeBibFile:
    ...
    def test_cyrillic_author_transliterated(self, tmp_path: Path):
        ...
        from surveyclaw.templates.compiler import _sanitize_bib_file
...
class TestRunBibtex:
    ...
    @patch("surveyclaw.templates.compiler.shutil.which", return_value="/usr/bin/bibtex")
    @patch("surveyclaw.templates.compiler.subprocess.run")
    def test_bibtex_failure_logged(self, mock_run, mock_which, tmp_path):
        """Failed bibtex should log warning and return False."""
        from surveyclaw.templates.compiler import _run_bibtex
    ...
    @patch("surveyclaw.templates.compiler.shutil.which", return_value=None)
