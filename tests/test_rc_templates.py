"""Unit tests for surveyclaw.templates — conference templates + MD→LaTeX converter."""
...
from surveyclaw.templates.conference import (
    CONFERENCE_REGISTRY,
    ConferenceTemplate,
    get_template,
    list_conferences,
...
from surveyclaw.templates.converter import (
    markdown_to_latex,
    _parse_sections,
    _extract_title,
    _extract_abstract,
...
class TestExportConfig:
    ...
    def test_default_values(self) -> None:
        from surveyclaw.config import ExportConfig
    ...
    def test_rcconfig_has_export(self) -> None:
        ...
        cfg = RCConfig.load("config.surveyclaw.example.yaml", check_paths=False)
    ...
    def test_rcconfig_export_from_dict(self) -> None:
        ...
        data = yaml.safe_load(Path("config.surveyclaw.example.yaml").read_text(encoding="utf-8"))
        data["export"] = {
...
class TestHitlStageValidation:
    ...
    def test_stage_23_valid(self) -> None:
        from surveyclaw.config import validate_config
        import yaml
        from pathlib import Path
