...
from surveyclaw import cli as rc_cli
from surveyclaw.config import resolve_config_path
from surveyclaw.pipeline.executor import StageResult
...
def test_main_with_no_args_returns_zero_and_prints_help(
    ...
    assert "SurveyClaw" in captured.out
...
def test_cmd_run_reports_paused_pipeline(
    ...
    from surveyclaw.pipeline import runner as rc_runner
...
def test_cmd_init_creates_config(
    ...
    monkeypatch.chdir(tmp_path)
    _write_example_config(tmp_path / "config.surveyclaw.example.yaml")
...
def test_cmd_run_missing_config_shows_init_hint(
    ...
    code = rc_cli.cmd_run(args)
    assert code == 1
    assert "surveyclaw init" in capsys.readouterr().err
...
def test_resume_finds_existing_checkpoint_dir(
    ...
    import surveyclaw.pipeline.runner as runner_mod
    ...
    import surveyclaw.llm as llm_mod
