"""SurveyClaw config loading and validation."""
...
EXAMPLE_CONFIG = "config.surveyclaw.example.yaml"
...
PROJECT_MODES = {"docs-first", "semi-auto", "full-auto", "survey"}
...
class SecurityConfig:
    hitl_required_stages: tuple[int, ...] = (5, 13)
...
class SshRemoteConfig:
    ...
    key_path: str = ""
    gpu_ids: tuple[int, ...] = ()
    remote_workdir: str = "/tmp/surveyclaw_experiments"
    remote_python: str = "python3"
    setup_commands: tuple[str, ...] = ()
    use_docker: bool = False
    docker_image: str = "surveyclaw/experiment:latest"
...
class ColabDriveConfig:
    ...
    drive_root: str = ""  # local mount path, e.g. ~/Google Drive/MyDrive/surveyclaw
...
class DockerSandboxConfig:
    ...
    image: str = "surveyclaw/experiment:latest"
...
class MemoryConfig:
    ...
    enabled: bool = True
    store_dir: str = ".surveyclaw/memory"
...
class KnowledgeGraphConfig:
    ...
    enabled: bool = False
    store_path: str = ".surveyclaw/knowledge_graph"
...
class MultiProjectConfig:
    ...
    enabled: bool = False
    projects_dir: str = ".surveyclaw/projects"
...
class RCConfig:
    ...
    # Experiment config (unused in survey mode, kept for API compatibility)
...
def validate_config(
    ...
    *,
    ...
    for key in REQUIRED_FIELDS:
        # ACP and Ollama providers don't need base_url or api_key_env
        if llm_provider in ("acp", "ollama") and key in ("llm.api_key_env",):
            continue
...
def _parse_llm_config(data: dict[str, Any]) -> LlmConfig:
    ...
    return LlmConfig(
        ...
        acp=AcpConfig(
            ...
            session_name=acp_data.get("session_name", "surveyclaw"),
...
def _parse_agentic_config(data: dict[str, Any]) -> AgenticConfig:
    ...
    return AgenticConfig(
        image=data.get("image", "surveyclaw/experiment:latest"),
...
def _parse_experiment_config(data: dict[str, Any]) -> ExperimentConfig:
    ...
    return ExperimentConfig(
        ...
        docker=DockerSandboxConfig(
            image=docker_data.get("image", "surveyclaw/experiment:latest"),
        ...
        ssh_remote=SshRemoteConfig(
            ...
            remote_workdir=ssh_data.get(
                "remote_workdir", "/tmp/surveyclaw_experiments"
            ),
            ...
            docker_image=ssh_data.get("docker_image", "surveyclaw/experiment:latest"),
...
def _parse_figure_agent_config(data: dict[str, Any]) -> FigureAgentConfig:
    ...
    return FigureAgentConfig(
        ...
        docker_image=data.get("docker_image", "surveyclaw/experiment:latest"),
...
def _parse_memory_config(data: dict[str, Any]) -> MemoryConfig:
    ...
    return MemoryConfig(
        enabled=bool(data.get("enabled", True)),
        store_dir=str(data.get("store_dir", ".surveyclaw/memory")),
...
def _parse_knowledge_graph_config(data: dict[str, Any]) -> KnowledgeGraphConfig:
    ...
    return KnowledgeGraphConfig(
        enabled=bool(data.get("enabled", False)),
        store_path=str(data.get("store_path", ".surveyclaw/knowledge_graph")),
...
def _parse_multi_project_config(data: dict[str, Any]) -> MultiProjectConfig:
    ...
    return MultiProjectConfig(
        enabled=bool(data.get("enabled", False)),
        projects_dir=data.get("projects_dir", ".surveyclaw/projects"),
...
def _parse_hitl_config(data: dict[str, object] | None) -> object | None:
    """Parse HITL config section."""
    return None
