...
Provides ``compile_latex()`` which prefers ``latexmk -pdf`` (for reliable
bibliography/citation resolution), then falls back to ``pdflatex`` + ``bibtex``
passes when latexmk is unavailable. It parses logs for common errors, applies
automated fixes, and retries up to 3 times. Designed to run inside
``_package_deliverables()`` so the final ``paper.tex`` in ``deliverables/``
is compile-tested.

If no suitable LaTeX compiler is installed the module gracefully returns a
failure report without raising.
"""
...
def compile_latex(
    ...
    *,
    ...
    """Compile *tex_path* with latexmk (or pdflatex fallback), auto-fixing errors.
    ...
    if not has_latexmk and not has_pdflatex:
        return CompileResult(
            success=False,
            log_excerpt="Neither latexmk nor pdflatex found on PATH",
            errors=["latexmk/pdflatex not installed"],
        )
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        if has_latexmk:
            # Prefer latexmk: it orchestrates pdflatex+bibtex reruns until
            # bibliography and cross-references converge.
            log_text, _ = _run_latexmk(work_dir, tex_name, timeout)
            if log_text is None:
                result.errors.append(
                    f"latexmk failed to execute (attempt {attempt})"
                )
                break
        else:
            # --- Full 3-pass fallback: pdflatex → bibtex → pdflatex × 2 ---
            ...
            log_text, _pass1_ok = _run_pdflatex(work_dir, tex_name, timeout)
            if log_text is None:
...
def _run_latexmk(
    ...
    """Run latexmk in PDF mode and return ``(log_text, success)``.

    Uses ``-interaction=nonstopmode`` to continue producing logs/PDF when
    possible, mirroring pdflatex behavior used elsewhere in the pipeline.
    """
    try:
        proc = subprocess.run(
        ...
        logger.warning("latexmk timed out after %ds", timeout)
        return None, False
    except FileNotFoundError:
        return None, False
    stdout = proc.stdout.decode("utf-8", errors="replace")
