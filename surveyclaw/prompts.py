"""Prompt externalization for the AutoSurveyClaw 16-stage survey pipeline.

All 16 survey-pipeline stage prompts are defined here as defaults and can be
overridden via a user-provided YAML file.  Users customize prompts without
touching Python source code.

Stage prompts cover the full survey pipeline:
  Phase A (Scoping):      topic_init, problem_decompose
  Phase B (Literature):   search_strategy, literature_collect, literature_screen,
                          knowledge_extract
  Phase C (Synthesis):    synthesis, taxonomy_build
  Phase D (Writing):      paper_outline, paper_draft, peer_review, paper_revision
  Phase E (Finalization): quality_gate, knowledge_archive, export_publish
...
* ``_DEFAULT_SUB_PROMPTS`` — secondary prompts (hypothesis synthesis, etc.).
        ...
        for stage_name, stage_data in (data.get("stages") or {}).items():
            if isinstance(stage_data, dict):
                if stage_name in self._stages:
                    ...
                    # Allow adding new stages from custom YAML (e.g. taxonomy_build)
...
SECTION_WORD_TARGETS: dict[str, tuple[int, int]] = {
    ...
    "taxonomy": (2000, 3000),
    "taxonomy and categorization": (2000, 3000),
    "detailed review": (4000, 6000),
    "detailed review of approaches": (4000, 6000),
    "comparative analysis": (2000, 3000),
    "comparative analysis and discussion": (2000, 3000),
    "open challenges": (1500, 2500),
    "open challenges and future directions": (1500, 2500),
    "future directions": (1500, 2500),
    "survey methodology": (500, 700),
    "background and preliminaries": (600, 900),
    "prior surveys": (600, 900),
}
...
_SECTION_TARGET_ALIASES: dict[str, str] = {
    ...
    # Survey aliases
    "taxonomy and categories": "taxonomy and categorization",
    "categorization": "taxonomy and categorization",
    "review of approaches": "detailed review of approaches",
    "literature review": "detailed review of approaches",
    "analysis and discussion": "comparative analysis and discussion",
    "comparative discussion": "comparative analysis and discussion",
    "challenges and future work": "open challenges and future directions",
    "future work": "open challenges and future directions",
    "open problems": "open challenges and future directions",
    "methodology": "survey methodology",
    "search methodology": "survey methodology",
    "background": "background and preliminaries",
    "preliminaries": "background and preliminaries",
    "related surveys": "prior surveys",
    "existing surveys": "prior surveys",
    "limitations of this survey": "limitations",
}
...
_DEFAULT_STAGES: dict[str, dict[str, Any]] = {
    ...
    "topic_init": {
        "system": (
            "You are a rigorous survey research planner who designs comprehensive "
            "academic literature surveys. You have deep expertise in systematic "
            "review methodology and know how to scope surveys that are neither "
            "too narrow (missing important work) nor too broad (unfocused)."
        ),
        "user": (
            "Create a comprehensive SMART survey goal in markdown.\n"
            "Topic: {topic}\n"
            "Domains: {domains}\n"
            "Project: {project_name}\n"
            "Quality threshold: {quality_threshold}\n\n"
            "Required sections:\n"
            "- **Topic**: Clear statement of the survey subject\n"
            "- **Scope**: What is included and excluded (specific technologies, "
            "time periods, problem settings, application domains)\n"
            "- **SMART Goal**: Specific, Measurable, Achievable, Relevant, Time-bound\n"
            "- **Inclusion Criteria**: Types of papers to include "
            "(peer-reviewed, relevance threshold, recency requirements)\n"
            "- **Exclusion Criteria**: What to explicitly exclude\n"
            "- **Constraints**: Target paper count, quality threshold\n"
            "- **Success Criteria**: What a high-quality survey of this topic looks like\n"
            "- **Generated**: Timestamp"
        ),
    },
    "problem_decompose": {
        "system": "You are a senior research strategist specializing in academic survey design.",
        "user": (
            "Decompose this survey topic into at least 5 prioritized sub-questions "
            "and search themes.\n"
            "Topic: {topic}\n\n"
            "Output markdown with sections:\n"
            "- **Key Survey Questions**: What the survey must answer\n"
            "- **Search Themes**: Themes to guide literature search\n"
            "- **Taxonomy Directions**: Potential ways to organize the literature\n"
            "- **Expected Coverage**: Key methods, benchmarks, application areas to cover\n"
            "- **Risks**: Coverage gaps, fast-moving sub-fields\n\n"
            "Goal context:\n{goal_text}"
        ),
    },
    ...
    "synthesis": {
        "system": "You are a synthesis specialist for comprehensive academic survey papers.",
        "user": (
            "Produce a deep synthesis of the reviewed literature.\n\n"
            "Output markdown with sections:\n"
            "- **Thematic Clusters** (3-6 clusters, each with: name, description, "
            "key papers, key findings)\n"
            "- **Cross-Paper Insights** (5+ insights that emerge from comparing "
            "multiple papers)\n"
            "- **Research Gaps** (3+ specific gaps with explanation of why important)\n"
            "- **Conflicting Findings** (where papers disagree and why)\n"
            "- **Taxonomy Directions** (how the literature can be organized)\n\n"
            "Topic: {topic}\n"
            "Knowledge cards:\n{cards_context}"
        ),
        "max_tokens": 8192,
    ...
    "taxonomy_build": {
        "system": (
            "You are an expert in constructing formal taxonomies for academic survey papers. "
            "Your taxonomy must:\n"
            "1. Be COMPREHENSIVE: Cover all major approaches in the field.\n"
            "2. Be PRINCIPLED: Categories based on clear, consistent criteria.\n"
            "3. Be HIERARCHICAL: Top-level categories with meaningful sub-categories.\n"
            "4. Be ACTIONABLE: Help readers find papers relevant to their needs.\n"
            "5. Be NOVEL: Offer a fresh organizational perspective, not just alphabetical listing."
        ),
        "user": (
            "Build a comprehensive taxonomy for organizing the literature on this topic.\n\n"
            "Output markdown with sections:\n"
            "1. **Taxonomy Overview**: The top-level organization principle and rationale\n"
            "2. **Category Definitions**: Each category with: name, definition, "
            "distinguishing criteria, key papers\n"
            "3. **Hierarchy**: Show the full taxonomy tree (use indented lists)\n"
            "4. **Paper Classification Table**: Markdown table mapping paper titles "
            "to their categories\n"
            "5. **Cross-Cutting Themes**: Themes that span multiple categories\n"
            "6. **Open Challenges by Category**: What remains unsolved in each category\n\n"
            "Requirements:\n"
            "- At least 3 top-level categories, ideally 4-6\n"
            "- Each category must have at least 2 representative papers or sub-categories\n"
            "- Categories must be mutually exclusive and collectively exhaustive within scope\n"
            "- Include both classic/foundational papers and recent work in each category\n\n"
            "Topic: {topic}\n\n"
            "Synthesis:\n{synthesis}"
        ),
        "max_tokens": 8192,
    ...
    "paper_draft": {
        "system": (
            "You are a PhD-level academic writer specializing in comprehensive survey papers "
            "for top venues (ACM Computing Surveys, IEEE TPAMI, Nature Reviews, etc.).\n\n"
            "KEY PRINCIPLES:\n"
            "1. COMPREHENSIVENESS: Cover all major sub-fields, seminal works, and recent advances.\n"
            "2. DEPTH: Go beyond listing papers — synthesize, compare, and critically analyze.\n"
            "3. STRUCTURE: Use a clear taxonomy to organize the literature logically.\n"
            "4. ATTRIBUTION: Every claim must cite specific papers; never state facts without sources.\n"
            "5. INSIGHT: Provide genuine analysis — what do the papers collectively tell us?\n"
            "6. GAPS: Explicitly identify what remains unsolved and why it matters.\n\n"
            "SURVEY-SPECIFIC RULES (CRITICAL):\n"
            "7. This is a SURVEY paper — do NOT invent new methods or algorithms.\n"
            "8. Do NOT include Method, Experiments, or Results sections.\n"
            "9. REQUIRED sections: Introduction, Background, Survey Methodology, Taxonomy, "
            "Detailed Review, Comparative Analysis, Open Challenges, Conclusion.\n"
            "10. Name open challenges with descriptive titles "
            "(e.g., 'The Generalization Gap Problem', 'Evaluation Inconsistency Challenge').\n"
            "11. Every factual claim must cite a paper using [cite_key] format.\n"
            "12. Include comparison tables wherever multiple methods are discussed.\n\n"
            "LENGTH REQUIREMENTS: Write 12,000-16,000 words total. "
            "Go deep on each category in the Detailed Review section."
        ),
        "user": (
            "{preamble}\n\n"
            "Write a COMPREHENSIVE, PhD-LEVEL survey paper draft in markdown.\n\n"
            "REQUIRED SECTIONS with minimum word counts:\n"
            "1. **Title** (informative, includes 'Survey' or 'Review')\n"
            "2. **Abstract** (250-350 words): scope, methodology, taxonomy, key findings\n"
            "3. **Introduction** (1000-1500 words): background, motivation, research questions, "
            "contributions, paper organization\n"
            "4. **Background and Preliminaries** (600-900 words): definitions, notation, "
            "foundational concepts\n"
            "5. **Survey Methodology** (300-500 words): search strategy, inclusion criteria, "
            "corpus statistics\n"
            "6. **Taxonomy** (800-1200 words): >=3 top-level categories with definitions, "
            "criteria, and representative papers per category\n"
            "7. **Detailed Review** (2500-4000 words): for each category, review key papers "
            "in depth covering: problem addressed, approach, evaluation, results, limitations. "
            "Include comparison tables.\n"
            "8. **Comparative Analysis** (800-1200 words): cross-method comparison on shared "
            "benchmarks, strengths/weaknesses, trade-off discussion\n"
            "9. **Open Challenges and Future Directions** (600-800 words): at least 5 named "
            "open problems with explanation of why they remain unsolved\n"
            "10. **Conclusion** (250-400 words): summary, key takeaways\n\n"
            "TOTAL TARGET: 12,000-16,000 words. Expand each section with substantive content.\n\n"
            "CRITICAL: Output raw markdown ONLY. Do NOT wrap in ```markdown``` code fences.\n"
    ...
    "peer_review": {
        "system": "You are an experienced academic reviewer specializing in survey papers.",
        "user": (
            "Simulate peer review from at least 2 reviewer perspectives for this survey paper.\n"
            "Output markdown with Reviewer A (coverage/breadth expert) and "
            "Reviewer B (depth/analysis expert), each with: strengths (3-5), "
            "weaknesses (3-5), actionable revisions, score (1-10).\n\n"
            "SURVEY REVIEW CRITERIA:\n"
            "1. COVERAGE: Does the survey comprehensively cover {topic}? "
            "Name any major sub-areas or important papers that are missing.\n"
            "2. TAXONOMY: Is the proposed taxonomy well-motivated and complete? "
            "Are the categories clear, non-overlapping, and collectively exhaustive?\n"
            "3. DEPTH: Does the paper synthesize and compare — or just list? "
            "Flag sections that are too superficial.\n"
            "4. COMPARATIVE ANALYSIS: Are methods compared on shared benchmarks "
            "with actual numbers? Is the analysis insightful?\n"
            "5. OPEN CHALLENGES: Are challenges specific, well-argued, and actionable? "
            "Generic challenges ('more research needed') are not acceptable.\n"
            "6. CITATION QUALITY: Are cited papers real and accurately described? "
            "Flag any suspicious or vague citations.\n"
            "7. WRITING: Is the survey well-organized, readable, and academically rigorous?\n"
            "8. SURVEY-SPECIFIC: Does the paper incorrectly present itself as a novel "
            "research paper rather than a survey? Flag any sections claiming original "
            "experiments, methods, or results not from cited papers.\n\n"
            "Paper draft:\n{draft}"
        ),
        "max_tokens": 8192,
    },
    "paper_revision": {
        "system": (
            "You are a survey paper revision expert.\n\n"
            "SURVEY REVISION PRINCIPLES:\n"
            "- NEVER shorten sections — only expand, deepen, and improve.\n"
            "- The revised survey MUST be at least as long as the draft.\n"
            "- This is a SURVEY paper — do NOT add Method or Experiments sections.\n"
            "- Expand coverage gaps identified by reviewers with specific paper discussions.\n"
            "- Refine the taxonomy if reviewers found it unclear or incomplete.\n"
            "- Deepen the comparative analysis with specific numbers and tables.\n"
            "- Make open challenges more specific and actionable.\n"
        ),
        "user": (
            "Revise the survey draft to address all review comments.\n"
            "Return revised markdown only. Start directly with the # Title heading.\n"
            "Do NOT wrap in code fences.\n\n"
            "CRITICAL REVISION RULES:\n"
            "- MAINTAIN OR INCREASE paper length (target 12,000-16,000 words total)\n"
            "- Each section must meet minimum word counts: "
            "Abstract (250-350), Introduction (1000-1500), Background (600-900), "
            "Taxonomy (800-1200), Detailed Review (2500-4000), "
            "Comparative Analysis (800-1200), Open Challenges (600-800), Conclusion (250-400)\n"
            "- CITATION FORMAT: All citations MUST remain in [cite_key] bracket format. "
            "Do NOT convert to author-year format.\n"
            "- CITATION KEYS: Do NOT invent new citation keys not in the draft.\n"
            "- Do NOT add Method, Experiments, or Results sections.\n"
            "- Do NOT claim original contributions — attribute all findings to cited papers.\n\n"
            "{topic_constraint}"
            "Draft:\n{draft}\n\nReviews:\n{reviews}"
        ),
        "max_tokens": 32768,
    ...
    "quality_gate": {
        "system": "You are a final quality gate evaluator for academic survey papers.",
        "user": (
            "Evaluate this survey paper quality and return JSON.\n"
            "Schema: {score_1_to_10: number, verdict: string (pass|revise|reject), "
            "criteria: {coverage: N, depth: N, taxonomy_quality: N, clarity: N, "
            "citation_quality: N}, strengths: [...], weaknesses: [...], "
            "required_actions: [...]}.\n\n"
            "Score each criterion 1-10. The overall score is the average.\n"
            "verdict: 'pass' if score >= threshold, 'revise' if score >= threshold-2, "
            "'reject' otherwise.\n\n"
            "Survey quality criteria:\n"
            "- coverage: Are major sub-fields, seminal works, and recent advances included?\n"
            "- depth: Does it synthesize and compare beyond just listing papers?\n"
            "- taxonomy_quality: Is the taxonomy well-motivated and comprehensive?\n"
            "- clarity: Is the writing clear, organized, and academically rigorous?\n"
            "- citation_quality: Are citations specific, accurate, and well-integrated?\n\n"
            "Threshold: {quality_threshold}\n"
            "Paper:\n{revised}"
        ),
        "json_mode": True,
    ...
    "knowledge_archive": {
        "system": "You produce survey retrospectives documenting methodology and findings.",
        "user": (
            "{preamble}\n\n"
            "Write a survey retrospective archive in markdown covering:\n"
            "1. Literature search methodology (databases, queries, screening criteria)\n"
