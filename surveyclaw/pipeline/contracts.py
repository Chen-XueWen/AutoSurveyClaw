"""Stage I/O contracts for the 16-stage SurveyClaw pipeline.
...
CONTRACTS: dict[Stage, StageContract] = {
    ...
    Stage.TOPIC_INIT: StageContract(
        ...
        input_files=(),
        output_files=("goal.md",),
        dod="SMART goal statement with survey topic, scope, and inclusion/exclusion criteria",
    ...
    Stage.SYNTHESIS: StageContract(
        ...
        dod="Thematic clusters + >=3 cross-paper insights + open challenges identified",
        error_code="E07_SYNTHESIS_WEAK",
    ),
    Stage.TAXONOMY_BUILD: StageContract(
        stage=Stage.TAXONOMY_BUILD,
        input_files=("synthesis.md",),
        output_files=("taxonomy.md",),
        dod="Formal taxonomy with >=3 categories and per-paper classification",
        error_code="E08_TAX_INVALID",
    ),

    # Phase D: Paper Writing
    Stage.PAPER_OUTLINE: StageContract(
        stage=Stage.PAPER_OUTLINE,
        input_files=("taxonomy.md",),
