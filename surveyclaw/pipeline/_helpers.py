...
from surveyclaw.hardware import HardwareProfile, is_metric_name
from surveyclaw.llm.client import LLMClient
from surveyclaw.pipeline.stages import (
    NEXT_STAGE,
    Stage,
    StageStatus,
)
...
_USER_SKILLS_DIR = Path.home() / ".surveyclaw" / "skills"
...
def _get_skill_registry(config: object | None = None) -> object:
    ...
    2. User-level ``~/.surveyclaw/skills/``
...
def _get_evolution_overlay(
    ...
    *,
    ...
    if run_dir is not None:
        try:
            from surveyclaw.evolution import EvolutionStore
    ...
    *,
    ...
    if include_goal:
        ...
        if goal:
            parts.append(f"\n### Goal\n{goal[:2200]}")
    if include_taxonomy:
        taxonomy = _read_prior_artifact(run_dir, "taxonomy.md")
        if taxonomy:
            parts.append(f"\n### Taxonomy\n{taxonomy[:2200]}")
...
def _topic_constraint_block(topic: str) -> str:
    """Return a hard constraint instruction that anchors survey content to the topic.

    Prevents the common LLM failure mode of drifting off-topic or
    writing a research paper instead of a survey.
    """
    return (
        "\n\n=== HARD TOPIC CONSTRAINT ===\n"
        f"The survey MUST comprehensively cover: {topic}\n"
        "PROHIBITED content:\n"
        "- Do NOT write about novel methods or original contributions — "
        "this is a SURVEY paper reviewing existing literature.\n"
        "- Do NOT present experimental findings as if they are your own — "
        "always attribute results to the cited papers.\n"
        "- Do NOT drift to tangential topics not directly related to the stated topic.\n"
        "- Every section MUST connect back to the survey scope.\n"
        "- The Abstract and Introduction MUST frame this as a comprehensive survey "
        f"of: {topic}\n"
        "- The paper MUST include a taxonomy, comparative analysis, and "
        "open challenges section.\n"
        "- Do NOT include 'Method' or 'Experiments' sections — "
        "use 'Review', 'Analysis', 'Taxonomy', 'Challenges' instead.\n"
        "=== END CONSTRAINT ===\n"
    )
...
def _generate_neurips_checklist(
    ...
    """Generate a survey paper quality checklist appendix in markdown.

    This checklist is based on standard survey paper quality criteria.
    It is appended to the paper before LaTeX conversion.
    """
    items = [
        ("Scope and Coverage", "Does the survey comprehensively cover the stated topic and scope?", "Yes"),
        ("Taxonomy", "Does the paper propose a clear taxonomy for organizing the literature?", "Yes"),
        ("Literature Search", "Is the literature search methodology described, including databases and query terms?", "Yes"),
        ("Inclusion Criteria", "Are inclusion/exclusion criteria for paper selection clearly stated?", "Yes"),
        ("Comparative Analysis", "Does the paper include a comparative analysis of reviewed methods?", "Yes"),
        ("Open Challenges", "Does the paper identify open challenges and future research directions?", "Yes"),
        ("Limitations", "Does the paper discuss limitations of this survey?", "Yes"),
        ("Citation Quality", "Are all cited papers real and accurately described?", "Yes"),
        ("Recency", "Does the survey cover recent work (last 3–5 years)?", "Yes"),
        ("Broader Impacts", "Are potential societal implications of the surveyed work discussed?", "Yes"),
    ]

    lines = [
        "## Survey Paper Quality Checklist",
        "",
    ]
...
def _generate_framework_diagram_prompt(
    ...
    *,
    ...
    if llm is not None:
        ...
        try:
            resp = _chat_with_prompt(llm, _system, _user, max_tokens=128_000)
...
def _default_taxonomy(topic: str) -> str:
    return f"""# Taxonomy of {topic}

## Category 1: Foundational Approaches
Papers establishing core methods, theoretical frameworks, or seminal benchmarks.

## Category 2: Methodological Variants
Papers proposing improvements, adaptations, or extensions to existing approaches.

## Category 3: Application Domains
Papers applying established methods to specific domains or problem settings.

## Category 4: Evaluation and Analysis
Papers focused on empirical comparison, ablation studies, or systematic analysis.

## Paper Classification
(Populated from knowledge cards during pipeline execution)

## Open Challenges
- Standardized benchmarking across settings
...
def _default_paper_outline(topic: str) -> str:
    return f"""# Survey Paper Outline: {topic}
...
- Head-to-head comparison across methods
...
def _default_quality_report(threshold: float) -> dict[str, Any]:
    ...
    return {
        ...
        "verdict": "revise",
        "criteria": {
            "coverage": round(min(10.0, score + 0.3), 2),
            "depth": round(score, 2),
            "taxonomy_quality": round(max(1.0, score - 0.2), 2),
            "clarity": round(min(10.0, score + 0.1), 2),
            "citation_quality": round(score, 2),
        },
        "strengths": [
            "Stage-by-stage evidence chain preserved",
            "Literature collection and synthesis artifacts are generated",
        ],
        "weaknesses": [
            "Coverage may need to include more recent papers",
            "Taxonomy categories may benefit from further refinement",
        ],
        "required_actions": [
            "Expand discussion of open challenges and future directions",
            "Verify all citations against real sources",
        ],
        "generated": _utcnow_iso(),
    }
...
def _multi_perspective_generate(
    ...
    *perspectives_dir* and returned as ``{role_name: response_text}``.
    """
    from surveyclaw.prompts import _render  # noqa: PLC0415
