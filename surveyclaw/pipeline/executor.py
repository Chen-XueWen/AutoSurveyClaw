...
from surveyclaw.hardware import HardwareProfile, detect_hardware, ensure_torch_available, is_metric_name
from surveyclaw.llm import create_llm_client
...
from surveyclaw.pipeline.contracts import CONTRACTS, StageContract
...
# ---------------------------------------------------------------------------
from surveyclaw.pipeline._domain import (  # noqa: E402
    _detect_domain,
    _is_ml_domain,
)
...
# ---------------------------------------------------------------------------
from surveyclaw.pipeline._helpers import (  # noqa: E402
    StageResult,
...
# ---------------------------------------------------------------------------
from surveyclaw.pipeline.stage_impls._topic import (  # noqa: E402
    _execute_topic_init,
...
# ---------------------------------------------------------------------------
from surveyclaw.pipeline.stage_impls._literature import (  # noqa: E402
...
# ---------------------------------------------------------------------------
from surveyclaw.pipeline.stage_impls._synthesis import (  # noqa: E402
    _execute_synthesis,
    _execute_taxonomy_build,
)
...
# ---------------------------------------------------------------------------
from surveyclaw.pipeline.stage_impls._paper_writing import (  # noqa: E402
...
# ---------------------------------------------------------------------------
from surveyclaw.pipeline.stage_impls._review_publish import (  # noqa: E402
...
def _run_hitl_pre_stage(
    ...
    from surveyclaw.hitl.intervention import HumanAction, PauseReason
...
def _run_hitl_post_stage(
    ...
    stage_num = int(stage)
    try:
        from surveyclaw.hitl.cost_guard import CostGuard
    ...
    if not session.should_pause_after(stage_num):
        ...
        try:
            from surveyclaw.hitl.smart_pause import SmartPause
...
def _run_collaboration_loop(
    ...
    *,
    ...
    """
    from surveyclaw.hitl.collaboration import CollaborationSession
    from surveyclaw.hitl.intervention import HumanAction, PauseReason
...
_STAGE_EXECUTORS: dict[Stage, Callable[..., StageResult]] = {
    ...
    Stage.TAXONOMY_BUILD: _execute_taxonomy_build,
    ...
    *,
    ...
    try:
        if config.llm.provider == "acp":
            ...
            if config.llm.provider in ("ollama", "local", "generic") or (
                candidate.config.base_url and candidate.config.api_key
            ):
                llm = candidate
        ...
        if (
            ...
            if mc_prm and getattr(mc_prm, "enabled", False):
                ...
                if int(stage) in prm_stages:
                    from surveyclaw.metaclaw_bridge.prm_gate import ResearchPRMGate
