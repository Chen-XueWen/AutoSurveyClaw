...
def _execute_literature_collect(
    ...
    *,
    ...
    try:
        from surveyclaw.data import load_seminal_papers
    ...
    if config.web_search.enabled:
        logger.info("Web search is disabled in this lean version.")
...
def _execute_knowledge_extract(
    ...
    *,
    ...
    _cards_written = False  # True when orchestrator already wrote card files

    # Parse shortlist rows for the paper reader
    shortlist_rows = _parse_jsonl_rows(shortlist)

    if llm is not None and shortlist_rows:
        try:
            from surveyclaw.agents.paper_reader import PaperReadingOrchestrator

            orch = PaperReadingOrchestrator(llm)
            result = orch.orchestrate({
                "papers": shortlist_rows,
                "topic": config.research.topic,
                "stage_dir": stage_dir,
            })
            cards = result.get("cards", [])
            if cards:
                _cards_written = True  # orchestrator wrote card files already
            logger.info(
                "Stage 6: %d knowledge cards extracted "
            ...
            logger.warning(
                "Stage 6: PaperReadingOrchestrator failed — "
                "falling back to abstract-only extraction",
                exc_info=True,
            )
            # Fallback: single-shot abstract-based extraction
            _pm = prompts or PromptManager()
            ...
            resp = _chat_with_prompt(
                ...
                json_mode=sp.json_mode, max_tokens=sp.max_tokens,
            )
            payload = _safe_json_loads(resp.content, {})
    ...
    # Write card files only when the orchestrator did not already write them
    # (orchestrator uses the richer schema; old loop uses legacy field names)
    if not _cards_written:
