...
from surveyclaw.pipeline._domain import _detect_domain, _is_ml_domain
...
def _strip_outer_code_fence(text: str) -> str:
    """Remove a wrapping ```markdown ... ``` (or ``` ... ```) block if present.

    LLMs sometimes wrap their entire markdown output in a code fence, which
    causes the fence delimiters to appear verbatim in the final paper.
    This strips the outer fence while preserving inner fences (tables, code).
    """
    stripped = text.strip()
    ...
    if fence_match:
        ...
        logger.info(
            "Stripped outer markdown code fence (%d→%d chars)", len(text), len(inner)
        )
        return inner.strip()
    return stripped
...
def _topic_is_literature_first(config: RCConfig) -> bool:
    ...
    """
    # For AutoSurveyClaw, we are ALWAYS literature-rich / survey-only.
    return True
...
def _execute_paper_outline(
    ...
    *,
    ...
    analysis = _read_prior_artifact(run_dir, "taxonomy.md") or _read_best_analysis(run_dir)
    ...
    if llm is not None:
        ...
        _is_survey_outline = _topic_is_literature_first(config)
        if _is_survey_outline:
            _survey_instruction = (
                "IMPORTANT: This is a SURVEY / LITERATURE REVIEW paper, NOT a novel-method paper.\n"
                "Do NOT invent new algorithms, techniques, or fictional method names.\n"
                "The outline MUST be structured around the taxonomy, categorization, "
                "and critical analysis of existing published work.\n"
                "Propose 3 candidate titles that clearly signal a survey, e.g. "
                "'A Survey of [Topic]', '[Topic]: A Comprehensive Review', "
                "'Advances in [Topic]: A Systematic Review' (each <= 14 words).\n"
                "The outline sections MUST include: Introduction, Related Surveys, "
                "Taxonomy and Categorization, Detailed Review of Approaches, "
                "Comparative Analysis, Open Challenges and Future Directions, Conclusion.\n\n"
            )
        else:
            _survey_instruction = (
        ...
        sp = _pm.for_stage(
            ...
            survey_instruction=_survey_instruction,
        )
    ...
    return StageResult(
        ...
        evidence_refs=(f"stage-{int(Stage.PAPER_OUTLINE):02d}/outline.md",),
    )
...
def _write_paper_sections(
    *,
    ...
    For survey papers (is_survey=True):
      Call 1: Title + Abstract + Introduction + Related Surveys
      Call 2: Methodology + Taxonomy + Detailed Review of Approaches
      Call 3: Comparative Analysis + Open Challenges + Limitations + Conclusion
    ...
    if is_survey:
        system = (
            "You are a top-tier academic author writing a SURVEY / LITERATURE REVIEW paper "
            "for a leading AI venue.\n\n"
            "CRITICAL RULES FOR SURVEY PAPERS:\n"
            "1. DO NOT invent new methods, algorithms, frameworks, or fictional technique names.\n"
            "2. The paper SYNTHESIZES and ORGANIZES existing published work — it does not propose new research.\n"
            "3. Structure the paper around a TAXONOMY of existing approaches.\n"
            "4. Compare and contrast methods from the collected literature using tables.\n"
            "5. All numerical results in tables MUST come from cited papers, not invented values.\n"
            "6. CITE extensively: a survey paper should cite 30-60+ papers.\n"
            "7. The conclusion identifies OPEN PROBLEMS and FUTURE DIRECTIONS, not your own contributions.\n"
            "8. WRITING QUALITY: Write as flowing academic prose — no bullet-point lists in main body.\n"
            "9. HONESTY: Accurately reflect what the surveyed literature says, without overclaiming.\n"
            "10. REPRODUCIBILITY: Clearly state the search strategy, inclusion/exclusion criteria, "
            "and time range covered by the survey.\n\n"
            "You ONLY use information from the collected literature — never fabricate citations or results."
        )
        if _overlay:
            system = f"{system}\n\n{_overlay}"
    else:
    ...
    if is_survey:
        call1_user = (
            ...
            "Write the following sections of an academic SURVEY / LITERATURE REVIEW paper "
            "in markdown. This is NOT a novel-method paper — do NOT invent new techniques, "
            "algorithms, or frameworks. The paper synthesizes and organizes existing published work.\n\n"
            "Follow the LENGTH REQUIREMENTS strictly:\n\n"
            "1. **Title** (HARD RULE: MUST be 14 words or fewer. The title MUST clearly signal "
            "this is a survey or review, e.g. 'A Survey of...', 'Advances in...', "
            "'[Topic]: A Comprehensive Review'. NEVER invent a fictional method acronym. "
            "NEVER use 'Untitled Paper'.)\n"
            f"2. **Abstract** (200-300 words): "
            f"State the scope of the survey, the body of literature covered, "
            f"the taxonomy or organizing framework, and key findings/gaps.){abstract_structure}\n"
            "3. **Introduction** (900-1200 words): motivate the importance of the survey topic, "
            "describe the scope and boundaries of the survey, state the research questions "
            "the survey addresses, outline the structure of the paper. "
            "MUST cite 8-12 references from the collected literature.\n"
            "4. **Related Work / Prior Surveys** (600-900 words): how this survey differs from "
            "existing surveys or reviews in the area. Discuss any prior surveys and their limitations.\n\n"
            f"Outline:\n{outline}\n\n"
            ...
            "The paper should read like a published survey manuscript, not a data report."
        )
    else:
        call1_user = (
            f"{preamble}\n\n"
    ...
    # Surveys need much more output space than research papers
    _paper_max_tokens = 64_000 if is_survey else 32_000
    ...
    if is_survey:
        call2_user = (
            ...
            f"{narrative_writing_rules}\n"
            f"{anti_hedging_rules}\n\n"
            "CITATION REQUIREMENT: You MUST cite at least 20-30 papers in this section. Use [cite_key] syntax.\n"
            f"{citation_instruction}\n\n"
            "You are continuing a SURVEY paper. The sections written so far are:\n\n"
            f"---\n{part1}\n---\n\n"
            "Now write the core analysis sections of the survey. "
            "This is the intellectual heart of a PhD-level survey — write with the depth and rigour of a "
            "prominent researcher who knows every paper in the field intimately.\n\n"
            "## Section requirements:\n\n"
            "5. **Methodology** (500-700 words): Describe the literature search and selection process:\n"
            "   - Search strategy: which databases/sources were queried (arXiv, Semantic Scholar, etc.)\n"
            "   - Search queries and keywords used\n"
            "   - Inclusion/exclusion criteria (venue quality, year range, relevance)\n"
            "   - Number of papers screened vs. included\n"
            "   - How the taxonomy/categorization scheme was developed\n"
            "   Write as prose, not bullets.\n\n"
            "6. **Taxonomy and Categorization** (2000-3000 words): "
            "Group the literature into a principled hierarchy of 3-5 top-level categories, "
            "each with 2-4 sub-categories where appropriate. For each category:\n"
            "   - Define the category clearly and explain WHY this grouping is meaningful\n"
            "   - List the key papers in that category with citations\n"
            "   - Explain the core technical idea shared by papers in this group\n"
            "   - Describe how this category relates to and differs from adjacent categories\n"
            "   Include a **summary table** listing the most important papers in each category "
            "with columns: Method/System, Key Contribution, Venue/Year, Citation Count (if known).\n\n"
            "7. **Detailed Review of Approaches** (4000-6000 words): "
            "For EACH major category from the taxonomy, write a dedicated subsection "
            "(use ### sub-headers) with 800-1500 words of analysis:\n"
            "   - **Seminal works**: Describe the foundational papers in depth — what problem they solved, "
            "     their technical approach (with relevant equations or pseudocode if illuminating), "
            "     their experimental results (metrics reported in the original paper), and their limitations.\n"
            "   - **Follow-up works**: How subsequent papers built on, improved, or challenged the seminal work.\n"
            "   - **Representative method comparison table**: A markdown table comparing the 4-8 most important "
            "     methods in this category with columns like: Method, Key Idea, Dataset/Benchmark, "
            "     Performance (as reported in paper), Limitations.\n"
            "   - **Critical analysis**: Do not merely describe — evaluate. What worked, what did not, "
            "     and why. Highlight surprising findings from the literature.\n"
            "   Write FLOWING ACADEMIC PROSE, not bullet lists. Each subsection must read like a "
            "   chapter section from a PhD thesis or a Nature/Science review article.\n\n"
            f"Outline:\n{outline}\n\n"
            "QUALITY BAR: Aim for the depth of a survey paper that would be cited 500+ times. "
            "Cover at least 25 distinct papers from the available references. "
            "Every major paper in the citation list should appear somewhere in this section.\n\n"
            "CRITICAL: DO NOT add notes like 'remaining sections omitted for brevity' or "
            "'the following sections would...' — write ALL the content in full. "
            "Do NOT truncate any section. If you are approaching the token limit, "
            "prioritize completing the Detailed Review of Approaches over other sections.\n"
            "CRITICAL: Do NOT add a '### Next Section:' note, a '---' separator followed by "
            "placeholder text, or any forward-reference to future sections. "
            "If you have content to write, write it NOW.\n"
            "CRITICAL: Output raw markdown ONLY. Do NOT wrap your response in markdown "
            "code fences (```markdown ... ``` or ``` ... ```) — output the content directly.\n\n"
            "Output markdown with ## and ### headers. Continue from where Part 1 ended."
        )
    else:
        call2_user = (
            f"{preamble}\n\n"
    ...
    part2 = _strip_outer_code_fence(part2)
    ...
    if is_survey:
        call3_user = (
            ...
            f"{anti_hedging_rules}\n"
            f"{anti_repetition_rules}\n\n"
            "CITATION REQUIREMENT: You MUST cite at least 10-15 papers when discussing trends and challenges.\n"
            f"{citation_instruction}\n\n"
            "You are completing a SURVEY paper. The sections written so far are:\n\n"
            f"---\n{part1}\n\n{part2}\n---\n\n"
            "Now write the final sections with PhD-level depth and rigour. "
            "This is where a great survey distinguishes itself — synthesise boldly, critique honestly, "
            "and guide the field forward.\n\n"
            "## Section requirements:\n\n"
            "8. **Comparative Analysis and Discussion** (2000-3000 words):\n"
            "   a. **Master comparison table**: Create a comprehensive markdown table comparing the "
            "      15-25 most important methods/papers covered in the survey. "
            "      Columns should include: Method/Paper, Category (from taxonomy), "
            "      Key Technical Innovation, Strengths, Weaknesses, Benchmark/Task, "
            "      Best Reported Performance (cite the paper), Year.\n"
            "      Bold the top performer in each benchmark column.\n"
            "   b. **Cross-cutting analysis**: Identify 4-6 overarching trends across all categories "
            "      (e.g., shift from X to Y over time, common failure modes, scalability patterns). "
            "      Cite specific papers as evidence for each trend.\n"
            "   c. **Trade-off analysis**: Explicitly discuss the core trade-offs in the field "
            "      (e.g., accuracy vs. interpretability, scalability vs. logical soundness). "
            "      Quantify where possible using reported numbers from cited papers.\n"
            "   d. **Consensus and controversies**: What does the community agree on? "
            "      Where do results conflict, and why might that be?\n"
            "   Write as flowing prose with the table embedded where most useful.\n\n"
            "9. **Open Challenges and Future Directions** (1500-2500 words): "
            "This section must be CONCRETE and VISIONARY — not a vague list of 'X needs more research'. "
            "For each challenge:\n"
            "   - State the challenge precisely, with evidence from the literature (cite papers)\n"
            "   - Explain WHY it is hard (technical, data, evaluation, or theoretical barriers)\n"
            "   - Propose 1-2 specific, actionable research directions to address it\n"
            "   - If promising preliminary work exists, cite it\n"
            "   Organise into 5-7 named challenges as ### subsections. Examples of good challenge framing:\n"
            "   '### Challenge 1: Scalability of Constraint Satisfaction to Large Search Spaces'\n"
            "   '### Challenge 2: Evaluation Benchmarks for Compositional Generalization'\n\n"
            "10. **Limitations of This Survey** (200-350 words): "
            "Honestly state what the survey does NOT cover, selection biases, recency cutoff, etc.\n\n"
            "11. **Conclusion** (300-500 words): "
            "Synthesise the survey's three most important takeaways in 2-3 sentences each. "
            "State clearly what the field has achieved and what remains unsolved. "
            "End with a forward-looking statement about the field's trajectory. "
            "Do NOT merely restate the abstract. Do NOT use bullet points.\n\n"
            "CRITICAL: DO NOT add placeholder notes or 'omitted for brevity' — "
            "write ALL content fully. If approaching the token limit, "
            "shorten the Conclusion but complete the Open Challenges section.\n"
            "CRITICAL: Output raw markdown ONLY. Do NOT wrap your response in markdown "
            "code fences (```markdown ... ``` or ``` ... ```) — output the content directly.\n\n"
            "Output markdown with ## and ### headers. Do NOT include a References section."
        )
    else:
        call3_user = (
            f"{preamble}\n\n"
    ...
    part3 = _strip_outer_code_fence(part3)
...
def _validate_draft_quality(
    ...
    *stage_dir*.
    """
    from surveyclaw.prompts import SECTION_WORD_TARGETS, _SECTION_TARGET_ALIASES
...
def _execute_paper_draft(
    ...
    *,
    ...
    _is_survey_draft = _topic_is_literature_first(config)
    preamble = _build_context_preamble(
        config,
        run_dir,
        include_goal=True,
        include_taxonomy=True,
        include_synthesis=True,   # Include thematic clusters for context
    )

    # SurveyClaw: Literature-focus only. Experiment data is not used for surveys.
    exp_metrics_instruction = ""
    has_real_metrics = False
    _verified_registry = None
    # --- Literature-rich framing for surveys ---
    exp_metrics_instruction = (
    ...
    logger.info("Stage 17: Literature-first mode enabled for AutoSurveyClaw")
    ...
    if llm is not None:
        ...
        draft = _write_paper_sections(
            ...
            is_survey=_topic_is_literature_first(config),
        )
        ...
        # Build template for survey
        results_section = "Template survey results sintetis based on literature."
        draft = f"""# Survey Draft Title
...
"""
    ...
    return StageResult(
        ...
        evidence_refs=(f"stage-{int(Stage.PAPER_DRAFT):02d}/paper_draft.md",),
    )
