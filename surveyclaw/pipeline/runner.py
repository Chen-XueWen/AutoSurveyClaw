...
from surveyclaw.evolution import EvolutionStore, extract_lessons
from surveyclaw.knowledge.base import write_stage_to_kb
from surveyclaw.pipeline.executor import StageResult, execute_stage
...
def _collect_content_metrics(run_dir: Path | None) -> dict[str, object]:
    ...
    draft_path = run_dir / f"stage-{int(Stage.PAPER_DRAFT):02d}" / "paper_draft.md"
    if draft_path.exists():
        try:
            quality_module = importlib.import_module("surveyclaw.quality")
    ...
    verify_path = run_dir / f"stage-{int(Stage.CITATION_VERIFY):02d}" / "verification_report.json"
...
def execute_pipeline(
    *,
    ...
    event_log = None
    try:
        from surveyclaw.pipeline.event_log import EventLog, EventType, create_event
    ...
    cost_budget = 0.0  # No cost budget tracking in survey mode
    ...
    for stage in STAGE_SEQUENCE:
        ...
        if cost_budget > 0:
            try:
                from surveyclaw.cost_tracker import get_global_tracker
...
def _package_deliverables(
    ...
    # Compute stage directories from the enum so renumbering doesn't break paths
    _export_stage = f"stage-{int(Stage.EXPORT_PUBLISH):02d}"
    _verify_stage = f"stage-{int(Stage.CITATION_VERIFY):02d}"

    # --- 1. Final paper (Markdown) ---
    # Prefer verified version (citation_verify stage) over base (export stage)
    paper_md = None
    for candidate in [
        run_dir / _verify_stage / "paper_final_verified.md",
        run_dir / _export_stage / "paper_final.md",
    ]:
    ...
    # --- 2. LaTeX paper ---
    # BUG-183: Export stage's paper.tex has been sanitized (fabricated numbers
    # replaced with ---).  Regenerating from Markdown would undo this because
    # the Markdown was never sanitized.  Prefer export-stage paper.tex when a
    ...
    _sanitization_report = run_dir / _export_stage / "sanitization_report.json"
    _was_sanitized = _sanitization_report.exists()
    verified_md = run_dir / _verify_stage / "paper_final_verified.md"
    if (
        not _was_sanitized
        ...
        try:
            ...
            from surveyclaw.pipeline.executor import _extract_paper_title
    ...
    if not tex_regenerated:
        tex_src = run_dir / _export_stage / "paper.tex"
        if tex_src.exists() and tex_src.stat().st_size > 0:
            shutil.copy2(tex_src, dest / "paper.tex")
            packaged.append("paper.tex")

    # --- 3. References (BibTeX) ---
    # Prefer verified bib (citation_verify stage) over base bib (export stage)
    bib_src = None
    for candidate in [
        run_dir / _verify_stage / "references_verified.bib",
        run_dir / _export_stage / "references.bib",
    ]:
        if candidate.exists() and candidate.stat().st_size > 0:
            bib_src = candidate
            break
    ...
    code_src = run_dir / _export_stage / "code"
    if code_src.is_dir():
        code_dest = dest / "code"
        if code_dest.exists():
    ...
    verify_src = run_dir / _verify_stage / "verification_report.json"
    ...
    san_src = run_dir / _export_stage / "sanitization_report.json"
    ...
    # --- 6. Charts (optional) ---
    charts_src = run_dir / _export_stage / "charts"
    ...
    try:
        from surveyclaw.templates import get_template
...
def _read_quality_score(run_dir: Path) -> float | None:
    ...
    report_path = run_dir / f"stage-{int(Stage.QUALITY_GATE):02d}" / "quality_report.json"
...
def execute_iterative_pipeline(
    ...
    *quality_threshold*, re-run stages 16-22 (paper writing + finalization) with
    ...
    for iteration in range(2, max_iterations + 1):
        ...
        reviews_text = ""
        reviews_path = run_dir / f"stage-{int(Stage.PEER_REVIEW):02d}" / "reviews.md"
        if reviews_path.exists():
            reviews_text = reviews_path.read_text(encoding="utf-8")
        _write_iteration_context(run_dir, iteration, reviews_text, score)

        # Re-run from PAPER_OUTLINE through EXPORT_PUBLISH
        logger.info(
            "Iteration %d/%d: re-running paper stages with feedback",
            iteration,
            max_iterations,
        )
        results = execute_pipeline(
...
def _metaclaw_post_pipeline(
    ...
    if l2s and getattr(l2s, "enabled", False) and lessons:
        try:
            from surveyclaw.metaclaw_bridge.lesson_to_skill import (
    ...
    try:
        from surveyclaw.metaclaw_bridge.skill_feedback import (
            SkillFeedbackStore,
            record_stage_skills,
        )
        from surveyclaw.metaclaw_bridge.stage_skill_map import get_stage_config
    ...
    try:
        from surveyclaw.metaclaw_bridge.session import MetaClawSession
