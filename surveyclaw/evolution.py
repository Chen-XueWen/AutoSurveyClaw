"""Self-evolution system for the SurveyClaw pipeline.
...
def _load_project_skills() -> list[str]:
    ...
    ``surveyclaw`` skill which is a CLI usage guide, not a pipeline skill).
    ...
    for rel_dir in _PROJECT_SKILLS_DIRS:
        ...
        for skill_sub in sorted(skills_dir.iterdir()):
            if not skill_sub.is_dir():
                continue
            # Skip the main surveyclaw CLI skill — it's not a pipeline overlay
            if skill_sub.name == "surveyclaw":
                continue
...
class EvolutionStore:
    ...
    def export_to_memory(self, memory_store: object) -> int:
        ...
        (compatible with ``surveyclaw.memory.store.MemoryStore``).
