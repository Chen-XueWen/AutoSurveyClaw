...
def verify_by_arxiv_id(arxiv_id: str, expected_title: str) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(url, headers={"User-Agent": "SurveyClaw/0.1"})
...
def verify_by_doi(doi: str, expected_title: str) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={
                "User-Agent": "SurveyClaw/0.1 (mailto:surveyclaw@example.com)",
                "Accept": "application/json",
            },
        )
...
_OPENALEX_EMAIL = "surveyclaw@users.noreply.github.com"
...
def verify_by_openalex(title: str) -> CitationResult | None:
    ...
    try:
        req = urllib.request.Request(
            url,
            headers={
                "User-Agent": f"SurveyClaw/0.1 (mailto:{_OPENALEX_EMAIL})",
                "Accept": "application/json",
            },
        )
...
_CACHE_DIR = Path.home() / ".cache" / "surveyclaw" / "citation_verify"
