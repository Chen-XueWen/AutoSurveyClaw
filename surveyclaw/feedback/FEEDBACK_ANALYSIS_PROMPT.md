...
> ```
> 请读取 surveyclaw/feedback/FEEDBACK_ANALYSIS_PROMPT.md，然后按照指示处理 feedback_inbox/ 目录下的所有测试反馈。
> ```
...
你是 AutoSurveyClaw 项目的高级 QA 工程师和代码架构师。你需要分析来自不同学科领域测试者的反馈，与当前 Pipeline 代码进行对比，生成一份结构化的 **Bug 修复文档**。

---

## 背景

AutoSurveyClaw 是一个 23 阶段的全自动学术研究 Pipeline（从选题到论文生成）。我们招募了来自不同学科的测试者运行 Pipeline，他们提交了运行反馈和交付物。你需要帮我把这些反馈转化为可执行的 Bug 修复方案。
...
- `surveyclaw/pipeline/stages.py` — 23 阶段定义和状态机
- `surveyclaw/pipeline/executor.py` — 核心执行逻辑（重点关注各阶段的 execute 函数）
- `surveyclaw/pipeline/runner.py` — Pipeline 运行入口
- `surveyclaw/config.py` — 配置结构
- `surveyclaw/llm/client.py` — LLM 调用逻辑
- `surveyclaw/literature/search.py` — 文献搜索
- `surveyclaw/experiment/docker_sandbox.py` — Docker 沙箱执行
- `surveyclaw/pipeline/code_agent.py` — 代码生成 Agent
- `surveyclaw/templates/converter.py` — LaTeX 转换
- `surveyclaw/prompts.py` — Prompt 模板
...
```markdown
# Bug Fix Document — AutoSurveyClaw Pipeline
